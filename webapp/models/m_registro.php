...
class M_registro extends MY_Model{
	...
	function InsertaInscripcion($datos){
		...
		//beneficiario
		$this->sql="insert into beneficiario (matricula_asignada, ap, am, nombre, id_solicitud,id_archivo, id_generacion_ps,fecha_carga, id_programa) values(:ps,:ap_p,:ap_m,:nombre,:id_solicitud,:id_archivo,:id_generacion,now(),:id_programa);";
 		$this->bindParameters($datos);
		$beneficiario = $this->db->query($this->sql);
		
		//b_personal
		$this->sql="insert into b_persona (matricula_asignada,ap_p, am_p, nombre_p,ap_m, am_m, nombre_m, celular, fecha_nacimiento, edad, id_sexo, email, curp, id_archivo, id_ocupacion,
										   id_estado_civil,id_grupo_etnico,id_hijos,finado_padre, finado_madre, id_lugar_nacimiento, id_discapacidad,petnica)
 							 		values(:ps,:apellidoPadreP, :apellidoPadreM, :nombrePadre,:apellidoMadreP, :apellidoMadreM, :nombreMadre, :celular, :fecha_nac, :edad, :sexo, :email, :curp, :id_archivo, :id_ocupacion,
										   :id_estado_civil,:id_etnia,:id_hijos,:finado_padre, :finado_madre, :lugar_nac, :id_discapacidad, :petnica);";
		...
		$this->sql="insert into b_escuela (matricula_asignada,matricula_escuela, id_institucion,id_plantel,id_turno,promedio,id_grado,id_sistema,num_mat_adeuda,id_archivo,id_carrera, id_generacion) 
									values(:ps,:matricula_escuela, :id_institucion,:id_plantel,:id_turno,:promedio,:id_grado,:id_sistema,:materias,:id_archivo,:id_carrera, :id_generacion);";
		$this->bindParameters($datos);
		...
		$this->sql="insert into b_dir (matricula_asignada,calle,noext,noint,ecalle,id_colonia,id_entidad,id_ut,id_archivo,ycalle,manzana,lote,edificio,rampa,andador,departamento,pasillo,villa,entrada,id_tiempo_residencia)
									values(:ps,:calle,:noext,:noint,:ecalle,:id_colonia,:lugar_nac,:id_uts,:id_archivo,:ycalle,:manzana,:lote,:edificio,:rampa,:andador,:departamento,:pasillo,:villa,:entrada,:id_tiempo_residencia);";
		$this->bindParameters($datos);
